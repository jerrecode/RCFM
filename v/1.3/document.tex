...
\label{sec:rmax}
The transition radius \(\Rmax\) is defined by the condition \(\rhoA(\Rmax)=\rhoc\), 1where \(\rhoc\) is fixed by the Ginzburg--Landau condensate (microphysics detailed in a companion paper). This removes \(\rhoc\) as a free parameter while deferring the exact electroweak identification.
...
Consider a test particle of Phase\~A (mass \(m_A\)) moving through the uniform Phase\~B medium of density \(\rhoB\). In the local flat approximation (valid for sub-horizon wakes, wavelength \(\ll\Rmax\)) the particle sources a metric perturbation \(\bar{h}_{\mu\nu}\) via the retarded Green function of the linearised Einstein operator. The force per unit volume on the entire Phase\~A fluid (\(n_A=\rhoA/m_A\)) is
\begin{equation}
...
For a Phase-A test particle moving at constant coordinate velocity the Fourier-space wake is
\[
\tilde{h}_{\mu\nu}(\mathbf{k})=-16\pi G\frac{\tilde{T}_{\mu\nu}(\mathbf{k})}{\omega^2-k^2+i\epsilon\omega}.
\]
The 4-force on a second test particle of Phase\~A is
\[
...
The force density on the whole Phase-A fluid is \(n_A\mathbf{F}_{\rm drag}\). Converting to the relativistic 4-vector transfer rate and symmetrising gives exactly Eq.\~(\ref{eq:Q-micro}) of Section\~\ref{sec:drag-micro}.
