...
def get_file_size(url):
    """HEAD request to pre-calculate size (for total progress and resume checks)."""
    try:
...
def main():
    ...
    for entry in manifest:
        ...
        if elapsed_total > 0 and done_bytes > 0:
            ...
            eta_seconds = remaining_bytes / (speed_mb_s * 1024 * 1024) if speed_mb_s > 0 else 0
            print(f"   Avg speed: {speed_mb_s:.2f} MB/s | ETA: {eta_seconds/60:.1f} min | Time elapsed: {elapsed_total/60:.1f} min")

    overall_bar.close()
    print("\nAll datasets downloaded successfully! Check raw/ folder and download_log.txt.")
