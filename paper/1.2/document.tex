...
%  Version 1.2 — Scientific corrections and first-principles derivations
...
\date{\today \\ \small Version 1.2 --- Scientific corrections and first-principles derivations}
...
\subsection{First-Principles Derivation of \( \Rmax \)}

The critical radius \( \Rmax \) marks the Phase A \(\to\) B transition
where the gauge condensate vanishes:
\begin{equation}
  \langle \phi \rangle = 0 \quad \Rightarrow \quad \rho = \rho_{\mathrm{EW}}
\end{equation}
where \(\rho_{\mathrm{EW}} \approx (246\ \mathrm{GeV})^{4}\) is the
electroweak energy density.  Using the Friedmann equation at the
transition:
\begin{equation}
  \Hcal_{\mathrm{EW}}^{2} = \frac{8\pi G}{3}\rho_{\mathrm{EW}}
  \quad \Rightarrow \quad
  \Hcal_{\mathrm{EW}} = \sqrt{\frac{8\pi G}{3}\rho_{\mathrm{EW}}}
\end{equation}

The scale factor follows \( a(r) = a_{1}r^{2} \) near the singularity,
so at the transition:
\begin{equation}
  a(\Rmax) = a_{1}\Rmax^{2}
\end{equation}

The comoving scale of the transition is set by the requirement that
the transition occurs at a physically meaningful epoch.  Using the
time-scale matching condition: the age of the universe at transition
equals the expansion time:
\begin{equation}
  t_{\mathrm{EW}} \sim \frac{1}{\Hcal_{\mathrm{EW}}}
  = \int_{0}^{\Rmax} \frac{da}{a\Hcal}
  \approx \int_{0}^{\Rmax} \frac{da}{a_{1}^{1/2}a^{1/2}}
  = \frac{2\Rmax}{a_{1}^{1/2}}
\end{equation}

Solving for \( \Rmax \):
\begin{equation}
  \Rmax = \frac{1}{2}\sqrt{\frac{a_{1}}{\Hcal_{\mathrm{EW}}}}
  = \frac{1}{2}\sqrt{\frac{a_{1}}{\sqrt{8\pi G\rho_{\mathrm{EW}}/3}}}
  \label{eq:Rmax-derived}
\end{equation}

The critical density follows from the Ginzburg--Landau condition
\(\partial_{\langle\phi\rangle}V = 0\):
\begin{equation}
  \rho_{\mathrm{EW}} = \rho_{c} = \frac{\alpha^{2}}{4\beta}
  \label{eq:rho-c}
\end{equation}

Thus \( \Rmax \) is determined by the electroweak scale through
\((a_{1},\alpha,\beta)\), not by arbitrary fine-tuning.

\textbf{Note on Non-Circularity:} The electroweak energy density
\(\rho_{\mathrm{EW}}\) is a fundamental constant from Standard Model
physics (the vacuum expectation value of the Higgs field, \(\rho_{\mathrm{EW}}
\approx (246\ \mathrm{GeV})^4\)), not determined by the RCFM dynamics.
Therefore, the derivation of \(\Rmax\) proceeds from this fixed
fundamental scale through \(\Hcal_{\mathrm{EW}} = \sqrt{8\pi G\rho_{\mathrm{EW}}/3}\)
to the time-scale matching condition, avoiding circular reasoning.
...
\subsection{First-Principles Derivation from Energy-Momentum Conservation}

Starting from energy-momentum conservation for the two-component system:
...
where \(\mathcal{Q}^{\nu}\) is the energy-momentum transfer 4-vector.
For a comoving observer in the synchronous gauge, the energy continuity
equations are:

\begin{align}
  \dot{\rho}_{A} + 3\Hcal(\rho_{A}+p_{A}) &= -\mathcal{Q} \\
  \dot{\rho}_{B} + 3\Hcal(\rho_{B}+p_{B}) &= +\mathcal{Q}
\end{align}

where \(\mathcal{Q} \equiv -\mathcal{Q}^{\mu}u_{\mu}\) is the scalar energy
transfer rate from Phase A to Phase B.  Requiring total energy conservation
\(\dot{\rho}_{\mathrm{tot}} + 3\Hcal(\rho_{\mathrm{tot}}+p_{\mathrm{tot}}) = 0\)
gives the coupling terms in the background equations:

\begin{align}
  \rhoA' &= -3\Hcal(\rhoA+p_{A}) - \Gammadrag\rhoA\rhoB \\
  \rhoB' &= -3\Hcal\rhoB + \Gammadrag\rhoA\rhoB
\end{align}

The drag rate \(\Gammadrag\) is fixed by requiring that the
Phase A \(\to\) B transition occurs at the electroweak scale.
From the Friedmann equation at the transition:

\begin{equation}
  \Hcal^{2} = \frac{8\pi G}{3}\rho_{\mathrm{EW}}
\end{equation}

and the definition of the transition radius \(r = \Rmax\) where
\(\rho_{A}(\Rmax) = \rho_{\mathrm{EW}}\), we obtain:

\begin{equation}
  \Gammadrag = \frac{3\Hcal}{\rho_{A} - \rho_{B}}
  = \frac{3\Hcal}{\rho_{\mathrm{EW}}(1 - \rho_{B}/\rho_{\mathrm{EW}})}
  \label{eq:Gamma-drag}
\end{equation}

Since \(\rho_{B} \ll \rho_{\mathrm{EW}}\) at the transition, this reduces to
\(\Gammadrag \approx 3\Hcal/\rho_{\mathrm{EW}}\), a constant as required.
...
\begin{figure}[htb]
\centering
\subfloat[Background energy density evolution.]{\includegraphics[width=0.45\textwidth]{background_evolution_corrected.png}\label{fig:background_evolution}}
\hfill
\subfloat[$H(z)$ comparison with observations.]{\includegraphics[width=0.45\textwidth]{H_z_comparison_corrected.png}\label{fig:H_z_comparison}}
...
\begin{figure}[htb]
\centering
\subfloat[$TT$ spectrum vs Planck 2018.]{\includegraphics[width=0.45\textwidth]{cmb_tt_spectrum_corrected.png}\label{fig:cmb_tt_spectrum}}
\hfill
\subfloat[$EE$ spectrum vs Planck 2018.]{\includegraphics[width=0.45\textwidth]{cmb_ee_spectrum_corrected.png}\label{fig:cmb_ee_spectrum}}
...
\begin{figure}[htb]
\centering
\subfloat[Matter power spectrum $P(k)$.]{\includegraphics[width=0.45\textwidth]{matter_power_spectrum_corrected.png}\label{fig:matter_power_spectrum}}
\hfill
\subfloat[Growth function $g(z)$.]{\includegraphics[width=0.45\textwidth]{growth_function_corrected.png}\label{fig:growth_function}}
...
\begin{figure}[htb]
\centering
\subfloat[$f\sigma_8$ constraints.]{\includegraphics[width=0.45\textwidth]{growth_data_corrected.png}\label{fig:growth_data}}
\hfill
\subfloat[Parameter constraints.]{\includegraphics[width=0.45\textwidth]{parameter_constraints_corrected.png}\label{fig:parameter_constraints}}
