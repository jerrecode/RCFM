...
\begin{enumerate}[label=(\roman*)]
  ...
  \item The singularity boundary condition \( a(r)\propto r^{2} \)
    ...
    primordial power spectrum, making inflation unnecessary.
...
\label{sec:rmax}
The transition radius \(\Rmax\) is defined by the condition \(\rhoA(\Rmax)=\rhoc\), where \(\rhoc\) is fixed by the Ginzburg--Landau condensate (microphysics detailed in a companion paper). This removes \(\rhoc\) as a free parameter while deferring the exact electroweak identification.
