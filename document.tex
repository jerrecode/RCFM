...
The effective potential for the vacuum expectation value (VEV) $⟨\phi⟩$ of the gauge condensate is:
\begin{equation}
...
The transition emits decoherence radiation—a stochastic GW burst at frequency \( f_{\mathrm{cycle}} \approx c / (2 \pi \lambda_{\mathrm{decoh}}) \), where \( \lambda_{\mathrm{decoh}} \approx \hbar / \sqrt{\rhoc} \) is the decoherence length.
...
\( \Lambda_{\mathrm{RCFM}} \) at different redshifts wiich are listed table number 1.
...
and\nobreakspace{}\ref{sec:eps1-derivation} are \( (\beta,\rho_{B,0}) \),
